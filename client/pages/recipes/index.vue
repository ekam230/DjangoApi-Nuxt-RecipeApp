...
export default {
  ...
  methods: {
    async deleteRecipe(recipe_id) {
      try {
        await this.$axios.$delete(`http://localhost:8000/api/recipes/${recipe_id}/`); // delete recipe
        let newRecipes = await this.$axios.$get("http://localhost:8000/api/recipes/"); // get new list of recipes
