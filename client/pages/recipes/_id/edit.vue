...
export default {
  ...
  async asyncData({ $axios, params }) {
    try {
      let recipe = await $axios.$get(`http://localhost:8000/api/recipes/${params.id}`);
      return { recipe };
    } catch (e) {
      return { recipe: [] };
    }
  ...
  methods: {
    ...
    async submitRecipe() {
      ...
      try {
        let response = await this.$axios.$patch(`http://localhost:8000/api/recipes/${editedRecipe.id}/`, formData, config);
